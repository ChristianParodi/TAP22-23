namespace TAP22_23.AuctionSite.Testing {
    public class AuctionTests : AuctionSiteTests {
        ...
        /// <summary>
        ///     Verify that if the second bid is the highest, the winner sets to the second bidder
        ///     and the price updates to the 2 highest + minimumBidIncrement
        /// </summary>
        [Test]
        public void Bid_NotFirstBidNotWinningOfferGtThanMaxOffer() {
            TheAuction.Bid(Bidder1Session, 100);
            TheAuction.Bid(Bidder2Session, 1000);
            Assert.Multiple(() => {
                Assert.That(TheAuction.CurrentWinner(), Is.EqualTo(Bidder2));
                Assert.That(TheAuction.CurrentPrice(), Is.EqualTo(100 + Site.MinimumBidIncrement));
            });
        }

        /// <summary>
        ///     Verify that if the second bid is not the highest and the bidder is not the current winner
        ///     the price sets  to the minimum between the second highest bid and the offer + minimumBidIncrement
        /// </summary>
        /// <param name="offer"></param>
        [Test]
        [TestCase(80)]
        [TestCase(100)]
        public void Bid_NotFirstBidNotWinningOfferLeThanMaxOffer(int offer) {
            TheAuction.Bid(Bidder1Session, 100);
            TheAuction.Bid(Bidder2Session, offer);
            Assert.That(TheAuction.CurrentPrice(), Is.EqualTo(Math.Min(100, offer + Site.MinimumBidIncrement)));
        }
    }
}