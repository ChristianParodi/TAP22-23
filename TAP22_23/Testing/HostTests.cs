namespace TAP22_23.AuctionSite.Testing {
    ...
    public class HostSessionCleanupTests : AuctionSiteTests {
        ...
        /// <summary>
        ///     Verify that the site name respects DomainConstraints.MinSiteName and DomainConstraints.MaxSiteName
        /// </summary>
        [Test]
        public void LoadSite_InvalidSiteNameLength() {
            var sitename = "";
            for (var i = 0; i <= DomainConstraints.MaxSiteName; i++)
                sitename += "a";
            Assert.That(() => TheHost.LoadSite(sitename), Throws.TypeOf<AuctionSiteArgumentException>());
        }
    }
}