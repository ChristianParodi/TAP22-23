namespace TAP22_23.AuctionSite.Testing {
    public class UserTests : AuctionSiteTests {
        ...
        public void Initialize() {
            ...
            Console.WriteLine(Site.ToyGetUsers());
        ...
        public void WonAuctions_UserWithWonAuctions_NonEmpty([Random(1, 10, 1)] int howManyAuctions) {
            ...
            for (var i = 0; i < howManyAuctions; i++) {
                ...
                var auction = sellerSession.CreateAuction($"The {i}th auction for {sellerName}",
                    TheClock!.Now.AddDays(randomGen.Next(3650)), startingPrice);
        ...
        /// <summary>
        ///     Verify that the Delete() on a user who owns ongoing auctions
        ///     throws exception
        /// </summary>
        [Test]
        public void Delete_UserOwnsOngoingAuctions() {
            var session = Site.Login(UserName, Pw);
            var auction = session!.CreateAuction("prova delete", new DateTime(2024, 1, 1), 85);
            Assert.That(() => User.Delete(), Throws.TypeOf<AuctionSiteInvalidOperationException>());
        }

        /// <summary>
        ///     Verify that the Delete() on a user who is winning an ongoing auction
        ///     throws exception
        /// </summary>
        [Test]
        public void Delete_UserIsWinningOngoingAuction() {
            Site.CreateUser("provolone", "passss");
            var tempUser = Site.Login("provolone", "passss");
            var auction = tempUser!.CreateAuction("ongoing", new DateTime(2024, 1, 1), 85);
            var currentUser = Site.Login(UserName, Pw);
            auction.Bid(currentUser, 1000);
